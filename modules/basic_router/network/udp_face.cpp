...
void UdpFace::send(const std::string &message) {
    _strand.dispatch(boost::bind(&UdpFace::sendImpl, shared_from_this(), ndn::Block((const uint8_t *)message.c_str(), message.length())));
}

void UdpFace::send(const ndn::Interest &interest) {
    _strand.dispatch(boost::bind(&UdpFace::sendImpl, shared_from_this(), interest.wireEncode()));
}

void UdpFace::send(const ndn::Data &data) {
    _strand.dispatch(boost::bind(&UdpFace::sendImpl, shared_from_this(), data.wireEncode()));
}
...
void UdpFace::sendImpl(const ndn::Block &block) {
...
void UdpFace::write() {
    const auto &block = _queue.front();
    _socket.async_send_to(boost::asio::buffer(block.wire(), block.size()), _endpoint,
